...
| # | 項目 | 期待効果 | 優先度 | 状態 |
|---|------|---------|--------|------|
| **21** | **Stage2: OV/EVルールにML閾値追加 (>=0.50)** | **FN -16件** | **高** | **✅実装完了 (2026-02-02)** |
| **19** | **Stage2: 危険TLD (.cfd) + ML >= 0.15 ルール** | **FN -5件** | **高** | **新規 (2026-02-02)** |
...
| 20 | Stage2: ブランド偽装 + ML >= 0.15 ルール | FN -8件 | 低 | 新規 (2026-02-02) |
| 17 | critical_brand_minimum ML閾値 | FP -22件 | 低 | **✅ 完了** |

#### #21 詳細（2026-02-02 追加）- 高ML + OV/EV問題 ✅ 検証完了

**仮説（検証済み）**: Stage1 MLモデルはDGAパターンを正しく検出しているが、Stage2のOV/EVルールが誤ってbenign判定している。

**検証結果**:
- 高ML (>= 0.50) のFN 19件中、**15件がOV/EVルールで誤判定**（79%）
- 発行者の多くがCloudflare（`has_organization=True`を自動設定）
- CRLルールはML < 0.30 にのみ適用されるため、高MLには影響なし

**根本原因（02_main.py line 1065-1077）**:
```python
# OV/EVルールにML閾値がない！
if cfg.get('stage2_cert_benign_ov_ev_enabled', False):
    has_org = X_defer[:, IDX_HAS_ORG] > 0.5
    ov_ev_mask = has_org.copy()  # ← ML閾値なし
...
| ドメイン | ML | 特徴 |
|---------|-----|------|
| tianhuashengrui.com | 0.974 | Cloudflare OV/EV |
| avigufsjvufhvxvv.com | 0.968 | ランダム文字列 + Cloudflare |
| aunanzon-co-jpsjf.com | 0.965 | Amazon偽装 + Cloudflare |
| amazon-user-center.net | 0.921 | Amazon偽装 |
| asdfghjjhggsf.moe | 0.887 | キーボードパターン |

**推奨修正（02_main.py）**:
```python
# OV/EVルールにML閾値を追加
if cfg.get('stage2_cert_benign_ov_ev_enabled', False):
    ov_ev_p1_max = float(cfg.get('stage2_cert_benign_ov_ev_p1_max', 0.50))  # 新規設定
    has_org = X_defer[:, IDX_HAS_ORG] > 0.5
    ov_ev_mask = has_org & (p1_defer < ov_ev_p1_max)  # ML閾値を追加
```

**期待効果**: FN -15件（Stage2）
**リスク**: 正規のOV/EV証明書サイト（ML >= 0.50）がStage3に送られる可能性 → 要評価

---

#### #19 詳細（2026-02-02 追加）

**背景**: Stage2 FN分析（424件）より、ML >= 0.15 かつ危険TLD (.cfd) のドメイン5件がフィルタされていた。

**現状の問題**:
- CRL（証明書失効リスト）が存在するため Stage2 で benign 判定
- しかし .cfd は HIGH_DANGER_TLD であり、フィッシングリスクが高い

**推奨アクション**:
- Stage2フィルタ条件に「危険TLD + ML >= 0.15 の場合は除外しない」ルールを追加
- 対象: `phishing_agent/pipeline.py` または `precheck` ロジック
...
**背景**: Stage2 FN分析より、ブランド偽装 + ML >= 0.15 のドメイン8件がフィルタされていた。

**現状の問題**:
- CRL存在でbenign判定されているが、ブランド偽装が検出されている
- Stage3なら検出できた可能性が高い

**推奨アクション**:
- Stage2フィルタ条件に「ブランド検出 + ML >= 0.15 の場合は除外しない」ルールを追加
- 実装複雑度が高いため、#19 の効果を見てから検討
...
- 2026-02-02: **#21実装完了** - `02_main.py` にOV/EVルールML閾値(0.50)を追加。期待効果: FN -16件, FPリスク +0~1件
- 2026-02-02: **Stage2 FN深掘り分析・#21検証完了** - 高ML FNの根本原因特定：OV/EVルールにML閾値がなく、Cloudflare証明書で誤benign判定。15件/19件（79%）がこれに該当。#21修正案確定（OV/EVルールにML<0.50閾値追加）
- 2026-02-02: **Stage2 FN分析完了** - 新パイプライン（RUN_ID: 2026-02-01_230749）でStage2 FN 424件を分析。フィルタ精度98.86%、FNの88.9%はML<0.05でモデル限界（受容すべき）、6.8%（29件）は改善可能と判定。#19/#20/#21タスク追加
...
## 16. 全件回帰テスト結果（2026-02-04）【SOエラー修正 + プロンプト改善後】
...
| 評価日 | 2026-02-04 |
| 評価件数 | 11,936件 |
| 対象 | ハンドオフ候補（Stage1+2が判断困難なケース） |
| 変更内容 | SOエラー修正 (max_length: 1000→2500) + プロンプト改善 (v1.7.1-prompt-recall) |

### 16.2 Stage3 (AI Agent) 性能
...
| 指標 | 前回 (2026-02-01) | 今回 (2026-02-04) | 変化 |
|------|-------------------|-------------------|------|
...
**注意**: 前回は15,630件、今回は11,936件と評価データが異なる（パイプライン更新による）。

### 16.5 TLD別性能（上位10）

| TLD | 件数 | Phishing | TP | FP | Precision | Recall | F1 |
|-----|------|----------|-----|-----|-----------|--------|-----|
| com | 6,607 | 1,163 | 624 | 205 | 75.3% | 53.7% | 62.7% |
| org | 1,077 | 20 | 12 | 31 | 27.9% | 60.0% | 38.1% |
| cn | 432 | 378 | 349 | 19 | 94.8% | 92.3% | 93.6% |
| net | 418 | 52 | 31 | 21 | 59.6% | 59.6% | 59.6% |
| top | 272 | 203 | 200 | 28 | 87.7% | 98.5% | 92.8% |
| ru | 187 | 5 | 3 | 9 | 25.0% | 60.0% | 35.3% |
| info | 145 | 17 | 8 | 20 | 28.6% | 47.1% | 35.6% |
| br | 143 | 16 | 5 | 6 | 45.5% | 31.2% | 37.0% |
| xyz | 109 | 51 | 46 | 33 | 58.2% | 90.2% | 70.8% |
| shop | 100 | 36 | 34 | 16 | 68.0% | 94.4% | 79.1% |

### 16.6 改善効果の分析

#### SOエラー修正の効果
- SOエラー率: 23.4% → 0.02%（ほぼ解消）
- reasoning長の87%が1,000文字超（旧制限ではエラー）

#### プロンプト改善の効果
- Recall: +3.97pp（見逃し削減）
- 高リスクケース（危険TLD + 高ML）で特に効果的
...
**重要**: Stage3がなければ、この1,691件のPhishingは全て見逃される。
Stage3は「難しいケース専門家」として機能している。

### 16.8 残課題

| 課題 | 詳細 |
|------|------|
| .org FP | Precision 27.9%と低い |
| .ru FP | Precision 25.0%と低い |
| .com Recall | 53.7%と低い |
...
- 2026-02-04: **全件回帰テスト完了** - SOエラー修正 + プロンプト改善後の評価。F1 71.90%（+3.03pp）、Recall 69.30%（+3.97pp）、FP 573件（-92件）、Stage3による Phishing検出 1,691件

...
## 16. OV/EV証明書の識別効果（大規模データ分析）

### 16.1 概要

SSL/TLS証明書は認証レベルによって以下の3種類に分類される：

| 認証レベル | 名称 | 検証内容 | コスト |
|-----------|------|---------|--------|
| DV | Domain Validation | ドメイン所有権のみ | 無料〜低額 |
| OV | Organization Validation | 組織の実在確認 | 中〜高額 |
| EV | Extended Validation | 組織の詳細確認 | 高額 |

OV/EV証明書は取得に組織の実在確認が必要なため、**フィッシングサイトでの使用は困難**と
考えられる。本セクションでは、640K件のフルデータセットを用いてこの仮説を検証した。
...
|------|------|
| 全ドメイン | 640,332 |
| 証明書データあり | 640,332 (100%) |
| ラベル付き（Stage1判定対象） | 127,754 |
...
| 認証レベル | 件数 | 割合 |
|-----------|------|------|
| DV（組織名なし） | 619,927 | **96.81%** |
| OV/EV候補（組織名あり） | 20,405 | 3.19% |
| ├ CDN経由（Cloudflare等） | 443 | 0.07% |
| └ **真のOV/EV** | **19,962** | **3.12%** |

**考察**: インターネット上のドメインの約97%がDV証明書を使用している。

#### OV/EV証明書の発行者内訳（上位10）

| 発行者 | 件数 | 割合 |
|--------|------|------|
| DigiCert Inc | 9,228 | 45.2% |
| Sectigo Limited | 2,611 | 12.8% |
| GlobalSign nv-sa | 2,402 | 11.8% |
| GEANT Vereniging | 955 | 4.7% |
| Entrust, Inc. | 591 | 2.9% |
| Internet2 | 412 | 2.0% |
| Cloudflare, Inc. | 378 | 1.9% |
| SSL Corporation | 350 | 1.7% |
| GoDaddy.com, Inc. | 300 | 1.5% |
| Cybertrust Japan Co., Ltd. | 280 | 1.4% |

### 16.4 真のOV/EV証明書のフィッシング率

#### CDN除外の重要性

Cloudflare等のCDNサービスは、プロキシ経由でアクセスされるサイトに対して
Cloudflare名義の証明書を発行する。これは**CDNの組織名であり、サイト運営者のOV/EV証明書ではない**。

| カテゴリ | 件数 | Phishing | Benign | Phishing率 |
|---------|------|----------|--------|-----------|
| CDN経由（Cloudflare等） | 79 | **73** | 6 | **92.4%** |
| 真のOV/EV（CDN除外） | 2,833 | 24 | 2,809 | **0.85%** |

**重要な発見**: CDN経由の「組織名あり」証明書はむしろ**高リスク**（92.4%がPhishing）。
これは、フィッシングサイトがCloudflareを経由してホストされるケースが多いためである。

#### 真のOV/EV（CDN除外）のラベル分布
...
**結論**: 真のOV/EV証明書を持つドメインは**99.15%がBenign**である。

### 16.5 Phishing + 真OV/EV の24件を分析

24件の「Phishing + 真OV/EV」を精査した結果、**全てが特殊なケース**であった：

| カテゴリ | 件数 | 例 |
|---------|------|-----|
| **侵害された正規サイト** | 15件 | `view.campanhasbradesco.com.br` (ブラジル銀行), `app.cgd.pt` (ポルトガル銀行CGD) |
| **フィッシングリンク共有プラットフォーム** | 5件 | `onedrive.live.com`, `assets-eur.mkt.dynamics.com` |
...
#### OV/EV証明書をBenign指標として使用可能

| 条件 | 判定 | 精度 |
|------|------|------|
| 真のOV/EV（CDN除外） | **Benign** | 99.15% |
| CDN経由（Cloudflare等）+ 組織名あり | 無視（信頼しない） | - |
| DV証明書 | 判別不可（追加分析必要） | - |

#### 実装案

```python
def is_safe_by_ov_ev(cert_info: dict) -> bool:
    """
    真のOV/EV証明書は安全と判定する。

    精度: 99.15% (2,809/2,833)

    注意: CDNプロバイダーの証明書は除外すること。
    """
    if not cert_info.get('has_organization'):
        return False
    ...
    issuer = cert_info.get('issuer_org', '')

    # CDNの場合は信頼しない
    for cdn in cdn_orgs:
        if cdn.lower() in issuer.lower():
            return False
...
#### Stage3データ（15,670件）での分布

| 認証レベル | 件数 | Phishing | Benign |
|-----------|------|----------|--------|
| DV | 15,663 | 2,733 | 12,930 |
| 真のOV/EV（CDN除外） | 1 | 0 | 1 |
| CDN経由 | 6 | 6 | 0 |

**考察**: Stage3データではOV/EV証明書が極めて稀（7件のみ）。
これは、Stage3が「グレーゾーン」で構成されており、OV/EV証明書を持つサイトは
Stage1/Stage2で既にBenign判定されている可能性が高い。
...
1. **大規模データによる定量的検証**:
   - 640K件のデータで「OV/EV証明書はフィッシングに使われない」仮説を検証
   - 真のOV/EV（CDN除外）でPhishing率**0.85%**という低い値を確認

2. **CDN除外の重要性の発見**:
   - CDN経由の「組織名あり」証明書はむしろ**高リスク**（92.4%がPhishing）
   - Cloudflare等のCDNサービスがフィッシングのホスティングに悪用されている

3. **侵害パターンの特定**:
   - Phishing + OV/EVの24件は全て「正規サービスの侵害・悪用」
   - **専用フィッシングドメインにOV/EV証明書を取得するケースは0件**

#### 限界

1. **カバレッジの低さ**: OV/EVは全ドメインの3.1%のみ
2. **侵害サイトの検出**: OV/EVでも侵害されたサイトは検出できない
3. **動的な変化**: 証明書は更新・変更されるため、定期的な再検証が必要

### 16.9 結論

**OV/EV証明書（CDN除外）は、高精度（99.15%）のBenign指標として使用可能である。**

ただし、以下の注意点がある：

1. **必ずCDNを除外すること** - Cloudflare等は高リスク
2. **侵害サイトには無力** - 正規サイトが乗っ取られたケースは検出不可
3. **カバレッジは限定的** - 全ドメインの3.1%のみが該当

この特徴量は「高精度・低カバレッジ」であり、
**他の特徴量（TLD、SAN数、CRL DP等）と組み合わせて使用すべき**である。

---

**作成日**: 2026-01-26
**更新日**: 2026-01-28（セクション16 OV/EV証明書分析追記）

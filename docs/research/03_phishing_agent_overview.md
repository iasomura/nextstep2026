...
| **POST_LLM_FLIP_GATE** | LLMがPHISH判定 + ml < 閾値（TLD依存） | is_phishing=False | LLMの過剰フリップを防止 |
| **POST_RANDOM_ONLY_GATE** | PHISH判定 + domain_issues={random_pattern}のみ + ブランドなし | is_phishing=False | ランダムパターンのみによる誤検知を防止 |
| **legit_tld_guard** | 正規TLD + 通常/長いドメイン | R1/R4の閾値を上げる | .com/.orgサイトの過剰検出を防止 |

#### 4.3.2 POST_LLM_FLIP_GATE（TLD依存閾値）（v1.6.2更新）

TLD危険度に応じた閾値でLLMのPHISH判定をブロック:

| TLD危険度 | 閾値 | 動作 |
|-----------|------|------|
| 高危険TLD | 0.0 | ゲート無効化（フィッシング検出最優先） |
| 中危険TLD | 0.04 | ほぼ無効（バランス重視） |
| 非危険TLD | 0.30 | FP削減重視 |

**設計根拠**: 3000件評価で非危険TLD閾値0.30によりF1が+4.81%改善（FP 142件削減、TP 26件損失）。

#### 4.3.3 Benign Certificate Gates（B1-B4）（v1.5追加）

証明書の正規性シグナルに基づき、PHISH判定をBENIGNに強制するゲート。

| ゲート | 条件 | 目的 |
|--------|------|------|
| **B1_OV_EV** | OV/EV証明書 + ctx < 0.50 | OV/EV証明書は組織確認済み |
| **B2_CRL** | CRL保有 + ml < 0.30 + ctx < 0.45 | CRLは正規CAの厳格プロセス |
| **B3_WILDCARD** | ワイルドカード + 非危険TLD + ctx < 0.40 | CDN/大規模サービス保護 |
| **B4_HIGH_SAN** | 高SAN数(≥10) + 非危険TLD + ctx < 0.45 | CDN/SaaS保護 |

**バイパス条件**: 強いリスクシグナル（self_signed, brand_detected, idn_homograph等）がある場合はスキップ。

#### 4.3.4 Low Signal Phishing Gates（P1-P3）（v1.5追加、v1.6.2更新）

低シグナルフィッシングを検出し、BENIGN判定をPHISHに強制するゲート。

| ゲート | 条件 | 動作 |
|--------|------|------|
| **P1_BRAND_SHORT_CERT** | ブランド検出 + 短期証明書(≤90日) + ml < 0.30 + 非危険TLDでない | PHISH強制 |
| **P2_SUSPECTED_COMPOUND** | ブランド疑い + 短期証明書 + 低SAN(≤5) + ml < 0.25 | PHISH強制 |
| **P3_DANGEROUS_TLD_CERT** | 危険TLD + 短期証明書 + 低SAN(≤3) + ml < 0.20 | risk_level bump |

**v1.6.2修正**: P1ゲートを非危険TLD + ML<0.30で無効化。
- 問題: POST_LLM_FLIP_GATEでブロックした判定をP1が再上書きし、FP 142件発生
...
| AI Agent（v1.6.1） | 670 | 276 | 139 | 0.7635 | 693 |
| AI Agent + S7戦略 | 628 | 107 | 181 | 0.8135 | 650 |

**S7戦略**: 2段階ゲート（非危険TLD + ML<0.30、危険TLD + ML<0.15）

---

## 10. FN（見逃し）分析と改善課題（2026-01-18）

### 10.1 FN分類

132件のCategory A FN（Stage1もAgentも見逃し）を分析:

| カテゴリ | 件数 | 割合 | 説明 |
|----------|------|------|------|
| **A1_BRAND_IMPERSONATION** | 14 | 10.6% | ブランド偽装検出で救済可能 |
| **A2_DANGER_TLD_MISSED** | 8 | 6.1% | 危険TLD検出ロジック確認で対応可能 |
| **A3_INVISIBLE_TO_ML** | 88 | 66.7% | XGBoost再学習が必要 |
| **A4_WEAK_SIGNALS** | 22 | 16.7% | コンテンツ分析など追加情報が必要 |

### 10.2 Stage3で救済可能なケース（22件）

#### A1: ブランド偽装（14件）

タイポスクワッティング/サブストリング検出強化で対応可能:

| ドメイン | 検出すべきブランド | ML確率 |
|----------|-------------------|--------|
| amaznoeom.com | amazon（タイポ） | 0.136 |
| americaexpre.com | amex（タイポ） | 0.087 |
| jibunbank.pro | じぶん銀行 | 0.126 |
| web-telegram.cc | Telegram | 0.012 |
| zoom-howto.com | Zoom | 0.082 |
| aifulbizd.sbs | アイフル | 0.458 |

**改善策**: brand_impersonation_checkのサブストリングマッチング強化、日本語ブランドDB拡充
...
|----------|-----|--------|
| g-jp.top | .top | 0.038 |
| jwaveyj.top | .top | 0.010 |
| yamzaklink.xyz | .xyz | 0.016 |
| inbook.cc | .cc | 0.023 |

**原因分析**: ML < 0.04（中危険TLD閾値）のためPOST_LLM_FLIP_GATEでブロック
**改善策**: 中危険TLDの閾値見直し、または他のシグナル（短期証明書+低SAN）との組み合わせ判定

### 10.3 Stage3で救済困難なケース（110件）

#### A3: ML不可視（88件、66.7%）

- 全件がML < 0.10（XGBoostが「確実に安全」と判定）
- 非危険TLD（.com, .br, .net等）
- 低エントロピー（単語らしいドメイン名）
- 証明書も通常（短期だが正規サイトも同様）

**必要な改善**:
1. XGBoostの再学習（新しい特徴量追加）
2. ドメイン登録日チェック（新規ドメインの検出）
3. コンテンツ分析（ログインフォームの検出等）

#### A4: 弱シグナル（22件、16.7%）

- ML 0.10〜0.30の範囲
- 個別のシグナルは弱いが、組み合わせで疑わしいケース

**必要な改善**:
1. コンテンツベースの分析強化
2. 証明書+TLD+ドメイン特徴の複合スコアリング
...
| 2 | 中危険TLD閾値調整 | +8件 | 低 |
| 3 | XGBoost再学習（新特徴量） | +88件（理論上） | 高 |
| 4 | コンテンツ分析ツール追加 | +22件（一部） | 中 |

**現実的な短期目標**: A1+A2の22件救済でFN率を22.4%→19.7%に改善

---

## 付録A: TLD危険度分類（v1.6.2更新）

2026-01-18時点のTLD危険度分類。POST_LLM_FLIP_GATEの閾値はこの分類に基づく。

### A.1 高危険TLD（HIGH_DANGER_TLDS）

フィッシングに特に多用され、正規利用が少ないTLD。ゲート閾値=0.0（ゲート無効化）。
...
### A.2 中危険TLD（MEDIUM_DANGER_TLDS）

フィッシングにも使われるが、正規ECサイト等でも使用されるTLD。ゲート閾値=0.04。
...
.com, .org, .net等の一般的なTLD。ゲート閾値=0.30（FP削減重視）。

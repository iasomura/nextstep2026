...
RUN_ID: 2026-01-15_222219 で新規データ生成・評価を実施。
...
| ドメイン | ML確率 | TLD危険度 | 特徴 |
|----------|--------|-----------|------|
| `3gs57a.lat` | 0.048 | HIGH | ランダム6文字+危険TLD |
| `jjbyjh.com` | 0.118 | LOW | ランダム6文字 |
| `postnordtud.top` | 0.143 | MEDIUM | **PostNordブランド偽装** |
| `youaishidai.com` | 0.088 | LOW | 中国語風ドメイン |
| `farmantisk.com` | 0.041 | LOW | 一般的なドメイン |
| `shangeketang.com` | 0.278 | LOW | 中国語風ドメイン |

**重要な発見:**
1. **PostNord偽装（postnordtud.top）が検出されていない**
   - PostNord（北欧郵便サービス）がブランドキーワードに含まれていなかった
   - TLDも.top（MEDIUM危険度）

2. **ランダムドメイン（3gs57a.lat）の見逃し**
   - HIGH危険TLDにもかかわらず検出失敗
   - Agent評価時にBENIGN判定
...
| ドメイン | ML確率 | TLD | 実態 |
|----------|--------|-----|------|
| `ccdrc.pt` | 0.857 | .pt | ポルトガルの正規組織 |
| `svenskalag.se` | 0.764 | .se | スウェーデンの正規サイト |
| `diamondgroupestates.com` | 0.648 | .com | 不動産会社 |

**分析:**
- 全てML確率が高い（0.65-0.86）
- ccTLD（.pt, .se）を持つ正規サイトが誤検出
- XGBoostモデルの過学習の可能性
...
`artifacts/2026-01-15_222219/models/brand_keywords.json` に以下を追加:

**郵便・配送サービス（17個）:**
- postnord, usps, fedex, ups, sagawa, yamato, japanpost
- royalmail, laposte, correos, deutschepost, auspost
- posteitaliane, canadapost, chronopost, colissimo

**日本の主要ブランド（14個）:**
- smbc, mufg, mizuho, aiful, docomo, linepay
- softbank, kddi, ekinet, mynumber, amazon, resona, aupay

### 全体FN内訳（128,067件テストデータ）

| カテゴリ | 件数 | 説明 |
|----------|------|------|
| **handoff_to_agent** | 1,440 | Agent評価済み→BENIGN判定 |
| **drop_to_auto** | 451 | Stage2で事前フィルタ |
| **not_candidate** | 2 | Stage1で自動判定 |
| **合計** | 1,893 | - |
...
`brand_keywords.json` が正しく読み込まれていなかった問題を修正:

```python
# scripts/evaluate_e2e.py の load_external_data() に追加
...
| FP | 3 | 3 | 0 |
| TN | 82 | 82 | 0 |
| Precision | 75.00% | **76.92%** | **+1.92%** |
| Recall | 60.00% | **66.67%** | **+6.67%** |
| F1 | 66.67% | **71.43%** | **+4.76%** |

#### PostNord検出成功

`postnordtud.top`: label=1 (フィッシング) → **final_pred=1 (正しく検出)**

- ML確率: 0.143 (低ML)
- TLD: .top (MEDIUM危険度)
- 検出要因: ブランドキーワード "postnord" のマッチ

#### 残存FN (5件)

| ドメイン | ML確率 | 検出困難な理由 |
|----------|--------|----------------|
| 3gs57a.lat | 0.048 | ランダムドメイン、ブランドなし |
| jjbyjh.com | 0.118 | ランダムドメイン、ブランドなし |
| youaishidai.com | 0.088 | 中国語風、ブランドなし |
| farmantisk.com | 0.041 | 一般的なドメイン |
| shangeketang.com | 0.278 | 中国語風、ブランドなし |

これらはブランド偽装パターンがなく、コンテンツ分析やWHOISデータなしでは検出困難。

### 次のステップ

1. **Stage2フィルタの改善**
   - drop_to_auto 451件の削減
   - 高ML+フィッシングが落とされる原因調査

2. **ccTLD過検出の改善**
   - .pt, .se などの正規ccTLDに対する保護ロジック追加

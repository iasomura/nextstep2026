...
Using the full-cascade rates from Table~X (e.g., $TPR=0.9818$, $FPR=0.00836$), we obtain the following projections.
...
For example, with $FPR=0.00836$ and $N_{\text{total}}=10{,}000$, $FP \approx 84$ per day (and $\approx 42$ per day at $p=0.5$).
...
\end{equation}
With $TPR=0.9818$ and $FPR=0.00836$, achieving $PPV^\star=0.90$ requires $p \gtrsim 0.0712$ (i.e., Benign:Phish $\lesssim 13:1$),
...
\end{equation}
With $TPR=0.9818$ and $PPV^\star=0.90$ (using system $FPR=0.00836$):
\begin{center}
\begin{tabular}{rcl}
\hline
...
\end{tabular}
\end{center}
Our system's $FPR=0.836\%$ is adequate under balanced evaluation but exceeds the required level by an order of magnitude at CT-log scale.

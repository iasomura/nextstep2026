...
### FP-3: `lightsystemsoft.com.br`（p₁ = 0.9571, handoff→救済）
...
### FP-4: `skatepro.com`（p₁ = 0.9571, handoff→救済）
...
| 根本原因 | 件数 | 方向 | 最終判定 |
|---------|------|------|---------|
| 稀な地理TLD（.wales, .scot）の訓練データ不足 | 2 | FP | auto_phishing（確定） |
| LE + 共有ホスティング証明書が正規サイトにも使用 | 2 | FP | handoff → 後段で救済 |
| 正規サイト乗っ取り（商用CA証明書がそのまま残存） | 1 | FN | auto_benign（確定） |
| 正規SaaS基盤の悪用 + 超短ブランド偽装ドメイン | 1 | FN | auto_benign（確定） |

## 4. 考察

### auto判定として確定した4件

6件中4件（FP-1, FP-2, FN-1, FN-2）はauto判定として確定し、Stage2/3に到達していない。これらはシステム全体で救済されない誤りである。

- **FP 2件（政府サイト）**: 稀なTLDに対する訓練データの追加、またはTLD長に対する特別な処理で改善の余地あり
- **FN 2件（正規インフラ悪用）**: 証明書・ドメイン特徴量のみでは原理的に検出困難。コンテンツ検査やURL構造の深い分析が必要

### 後段で救済された2件

FP-3（lightsystemsoft.com.br）とFP-4（skatepro.com）はhandoffとなり、Stage2/3で正しく救済された。これらはLE証明書 + 共有ホスティングという特徴が原因であり、Stage3のLLM + ルールが証明書以外の文脈情報を活用して正しく判定したと考えられる。

### 閾値設計の評価

127,222件中6件（0.005%）という高確信度誤判定率は、閾値（t_high=0.957, t_low=0.001）が十分に保守的に設定されていることを示す。ただし、auto判定確定の4件はシステム全体でも救済されないため、運用上のリスクとして認識が必要。

...
**前回の合意内容**: 第1層で0/1に無理に押し込むのではなく、確信度が低いサンプルを「グレー（判定保留）」とし、第2層に渡す。モデルはXGBoostに固執しない。
...
モデル比較として LightGBM・RandomForest とも比較し、F1差0.08pt以内であることを確認（付録A）。閾値運用によるグレー許容がモデル変更より効果的であったため、XGBoostを採用した。

**補足: 三値分類について**

前回の打ち合わせでは「三値分類（0=悪性, 1=正常, 2=グレー）」も選択肢として挙がった。今回は二値分類＋閾値運用で三値ルーティングを実現する方式を採った。三値ラベルでの学習は今後の検討課題として残っている。

---

### 2.2 決定②: 高確信度誤判定の特定・分析

**前回の合意内容**: 「自信を持って間違える」ケース（高確信度のFP/FN）を特定し、特徴分析によりチューニングして削減する。
...
さらに、Stage2で証明書ゲート（cert gate）+ LR誤り確率推定を追加し、自動判定の信頼性を補強した。自動判定（115,270件）中の誤りは**401件（0.348%）**に抑制。
...
**前回の合意内容**: バランスデータだけでなく、実分布（正規多数、フィッシング少数）でも評価し、精度の変化を確認する。
...
**前回の合意内容**: 第2層（エージェント）への入力は、確信度不足のグレー判定に限定する運用を目指す。
...
- Stage3追加で FNR -0.45pt 改善する一方、FPR +0.33pt 増加
- 自動判定誤り率 0.348% の運用上の許容性

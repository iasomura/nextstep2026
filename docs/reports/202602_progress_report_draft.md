...
|------|-----------|-------------|------|
| 精度 (Accuracy) | 95.70% | 99.00% | +3.30pp |
| 再現率 (Recall) | 93.42% | 99.00% | +5.58pp |
| 偽陰性率 (FNR) | 6.58% | 1.04% | **-5.54pp** |

### 3.2 処理フロー別の内訳

| フロー | 件数 | 割合 | 説明 |
|-------|------|------|------|
| auto_phishing | 59,274 | 9.3% | Stage1で高確信度phishing |
| auto_benign | 576,867 | 90.1% | Stage1で高確信度benign |
| handoff → Stage2/3 | 4,215 | 0.66% | グレー領域として精査 |

### 3.3 handoff候補での検出性能

Stage1がbenignと判定したがフィッシングだった4,215件に対し：

| 確信度範囲 | 件数 | AI Agent検出率 |
|----------|------|---------------|
| p < 0.10 | 1,145件 | 100% |
| 0.10 ≤ p < 0.20 | 1,051件 | 100% |
| 0.20 ≤ p < 0.30 | 736件 | 82.1% |
| 0.30 ≤ p < 0.50 | 1,283件 | 60.5% |

**考察**：森先生の指摘「自信を持って間違えるケースこそ問題」の通り、低確信度領域（p < 0.2）で最も効果を発揮
...
- 偽陰性率: 6.58% → **1.04%**（-5.54pp）
- 低確信度領域（p < 0.2）での検出率: **100%**
...
- FP削減（Precision 72.81% → 目標: 80%以上）
- FN削減（Recall 65.33% → 目標: 75%以上）

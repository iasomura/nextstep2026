...
|--------|-------------|-----|-----------|--------|-----|
| **XGBoost（採用）** | depth=8, lr=0.1, n=300 | **98.60%** | 99.48% | 97.73% | 0.51% |
...
F1差は最大0.08pt（LightGBM-B vs RF-A）。モデル間の差よりも、閾値運用によるグレー許容＋Stage3の追加（F1 98.60→98.67%）の方が寄与が大きいため、XGBoostを採用した。

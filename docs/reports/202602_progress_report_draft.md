...
```
              Predicted
            Benign  Phishing
Actual  Benign   63,560    317 (FP)
       Phishing  1,723  62,154 (TP)
```
...
| フロー | 件数 | 割合 | 説明 |
|-------|------|------|------|
| auto_phishing (p ≥ 0.5) | 62,471 | 48.9% | Stage1で高確信度phishing判定 |
| handoff (p < 0.5) | 65,283 | 51.1% | Stage2/Stage3へ委譲 |
...
Stage3の救済率（65.33%）を適用した推定値：

| 指標 | Stage1単体 | 3段階システム | 改善 |
|------|-----------|-------------|------|
| Precision | 99.49% | 94.63% | -4.86pp |
| Recall | 97.30% | 99.06% | **+1.76pp** |
| F1 Score | 98.39% | 96.80% | -1.59pp |
...
```
              Predicted
            Benign  Phishing
Actual  Benign   60,285   3,592 (FP)
       Phishing    598  63,279 (TP)
```

**考察**：
- FNRが2.70%→0.94%に改善（1,723件→598件）
- FPは増加（317件→3,592件）するが、Recallを優先する設計

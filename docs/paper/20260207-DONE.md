...
## Phase 2: 提出パッケージ準備（2026-02-07〜）

### [2026-02-07] P0-1: 文章・図表の言語/表記統一 ✅

**成果物**: `docs/paper/STYLE_GUIDE.md`
- 言語方針: 日本語本文 + 図も日本語（CSSは日本語論文）
- 用語の正規形: 12用語（投入率, 自動判定, 困難集合 等）を定義
- 数値表記ルール: 桁区切り・%・小数桁・閾値・処理時間
- 図ラベル方針: 日本語タイトル・軸・凡例・n表記・フォント
- 表の方針: 日本語ヘッダ（TP/FP/F1等は英略語）
- 英語論文にする場合の差分メモ付き

---

### [2026-02-07] P0-2: 最終採用図セットの確定 ✅

**成果物**:
- `docs/paper/images/` を最終採用品（5点）のみに整理
  - fig01_architecture.png（図1: アーキテクチャ）
  - fig02_transitions.png（図2: Stage遷移）
  - fig03_threshold_sweep.png（図3: 閾値スイープ）
  - fig04_latency.png/pdf（図4: 処理遅延CDF）
  - fig05_error_breakdown.png/pdf（図5: 誤り分析）
- 落選図8点を `_unused/` に隔離
- `FIGURE_MAP.md` を最終採用品のみに更新

---

### [2026-02-07] P0-3: Fig4（処理遅延CDF）生成 ✅

**成果物**: `docs/paper/images/fig04_latency.png` + `.pdf`
- CDF形式: 横軸=処理時間（秒）、縦軸=累積割合
- p50=8.31s, p90=15.27s, p99=28.59s の縦線注記
- n=11,952 を図中に明示
- STYLE_GUIDE準拠の日本語ラベル
- データソース: `fig4_processing_time.csv`
- 生成スクリプト: `scripts/generate_new_figures.py`

---

### [2026-02-07] P0-4: Fig5（誤り分析）生成 ✅

**成果物**: `docs/paper/images/fig05_error_breakdown.png` + `.pdf`
- 左右2パネル: FN Stage別分布 / FP Stage別分布
- FN: Stage1=3(0.3%), Stage2=395(34.1%), Stage3=760(65.6%), 合計1,158件
- FP: Stage1=2(0.4%), Stage2=1(0.2%), Stage3=529(99.4%), 合計532件
- Stage3 FN内訳（jpcert:387, phishtank:232, certificates:141）を注記
- STYLE_GUIDE準拠の日本語ラベル
- データソース: `fig5_error_categories.csv`
- 生成スクリプト: `scripts/generate_new_figures.py`

---

### [2026-02-07] P0-5: Stage1/ベースラインのハイパラ表記整合 ✅

**発見した不整合**:
- XGBoost `n_estimators`: 設定ファイル=300, 論文=100, ベースラインCSV/スクリプト=500
- 正値: `02_stage1_stage2/configs/default.yaml` の **n_estimators=300**

**修正箇所**:
1. `scripts/evaluate_baselines_minimal.py` line 110: "n=500" → "n=300"
2. `docs/paper/data/tables/appendix_baselines.csv`: XGBoost "n=500" → "n=300"
3. `docs/paper/paper_outline.md` line 254: "n_estimators=100" → "n_estimators=300" + 全パラメータ追記
4. `docs/paper/number_source_map.md`: ハイパラ出典セクション追加（default.yaml参照）

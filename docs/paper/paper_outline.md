...
| データ規模 | 640,356件 | 学習+評価 638,766件（Train 511,012 / Test 127,754準備→127,222評価、50:50バランス） |
...
- 基本条件（safe_benign判定）: Stage1確率 p1 < 0.15 かつ LR推定の defer_score < 0.40 を満たすhandoff候補をbenignとして自動判定する
- 主メカニズム: LRによる p_error 推定
  - 入力: Stage1と同等の42特徴量 + Stage1出力由来特徴（p1、エントロピー、マージン等の不確実性指標）
  - 目的変数: Stage1判定が誤りか否か
  - 学習: Stratified CVとOOF予測で閾値選定（データリーク抑制）
- 補助メカニズム: 証明書属性ゲート（6ルール）
  - Benignゲート4件: (1) CRL配布点あり+ML<0.30、(2) OV/EV証明書（組織情報あり）+ML<0.50、(3) ワイルドカード証明書+非危険TLD、(4) 長期有効期間（>180日）+ML<0.25
  - Phishingゲート2件: (5) Tier1危険TLD+Let's Encrypt、(6) 動的DNSサフィックス+高SAN数
  - 目的: LRの誤りを、決定的条件で抑制または補強
- TLDリスク制御（4カテゴリ: safe/dangerous/neutral/other）
  - 危険TLDではbenignゲートを無効化し、過剰な正常判定を抑制する
  - 中立TLDではML閾値条件を厳格化（p1 < 0.03のみ適用）

#### 3.4 Stage3: LLMエージェントとルールエンジンの統合
- AI Agent構成
  - LangGraph StateGraphによる7ノードワークフロー（precheck → tool_selection → fanout → tool_execution → aggregate → contextual_check → final_decision）
  - 小型LLM: Qwen3-4B-Thinking GPTQ-Int8量子化（temperature=0.1）、vLLM推論サーバで運用
  - 4種分析ツール（ブランド偽装検出、証明書解析、短ドメイン分析、コンテキストリスク評価）
  - Structured Output（PhishingAssessmentSO）: is_phishing, confidence, risk_level, risk_factors, reasoning, detected_brands, primary_category（8分類）, mitigated_risk_factors
- ドメイン知識ルール（ポストプロセッサ: 38ルールクラス、12モジュール）
  - 目的: LLM判定を専門知識で補正し、再現性と説明可能性を担保
  - モジュール構成:
    - ML Guard（5件）: 高確信度オーバーライド、超低スコア保護、コンテキスト救済、POST_LLM反転ゲート
    - Policy（6件）: MLスコア帯×コンテキストリスクの複合判定
    - 証明書ゲート（4件）: benign証明書属性による誤検知抑制
    - 低シグナルゲート（4件）: 低シグナル時のphishing判定保護
    - ブランド+証明書（2件）: ブランド偽装+低品質証明書の複合条件
    - CTXトリガー（2件）: コンテキストリスク閾値による強制判定
    - TLDコンボ（4件）、ML Paradox（2件）、低シグナル（2件）、Gov/Edu（1件）、ポストゲート（6件）
  - モジュール化: 個別ルールの有効化・無効化、閾値調整、発火ログの保存
  - ルール設計方針:
    - 全ルールは証明書・TLD・ブランド偽装に関するドメイン知識に基づいて設計
    - 例: 「無料DV証明書 + 組織情報なし」はフィッシングサイトに共通する既知の特徴であり、証明書認証レベルの階層（DV < OV < EV）は業界標準の信頼指標である
    - 閾値（ctx_score等）はドメイン知識で設計した条件の判定感度を調整するパラメータであり、データからパターンを発掘したものではない
...
- Train/Testは80:20で分割し、Test準備段階で127,754件（Phishing 63,877 / Benign 63,877）。ただし特徴量抽出パイプラインで532件が脱落し、実際の評価対象は**127,222件（Phishing 63,611 / Benign 63,611）**である。
- ソース別内訳（Test準備段階、合計127,754）:
  - certificates 38,044
  - jpcert 22,401
  - phishtank 3,432
  - trusted 63,877
- 注: 脱落の主因は証明書のパース失敗や特徴量算出時の欠損。以降の評価結果は127,222件に基づく。
...
- Stage3: Qwen3-4B-Thinking GPTQ-Int8（vLLM推論サーバ、temperature=0.1, max_tokens=512） + 38ルールエンジン
...
- 基本性能（Test 127,222件）
  - auto_phishing: 60,767件 (47.8%)、TP=60,765, FP=2
  - auto_benign: 8,464件 (6.7%)、TN=8,461, FN=3
  - handoff: 57,991件 (45.6%)、Phishing 2,843, Benign 55,148
  - 自動判定率: 54.4%（69,231/127,222）、自動判定誤り: 5件（FP 2 + FN 3）
- 特徴量重要度
- handoff候補分布（phishing/benignの内訳）

#### 4.3 Stage2: LR + ゲートの効果
- handoff 57,991件からの振り分け:
  - drop_to_auto（safe_benign等）: 46,039件 → うちFN=395件（0.86%の phishing 見逃し）
    - base safe_benign: 32,179件（p1<0.15 AND defer<0.40）
    - 証明書ゲート追加分: 13,128件（CRL: 43,310, OV/EV: 688, Wildcard: 2,658, LongValidity: 5,250 ※重複あり、net: 45,171）
    - Phishingゲート: 8件（Tier1 TLD+LE）
  - handoff_to_agent: 11,952件 → Stage3へ送付
- p_error推定性能（AUC、PR-AUC、キャリブレーション）
- Stage3投入率の削減効果: 57,991件中 11,952件のみ送付（投入率 20.6%、全体の9.4%）
- 自動判定の誤り率（drop_to_auto内 FN=395, FP=1）
- アブレーション（Stage2分解、実装のScenario構成に対応）:
  - Stage1のみ（全handoffをStage3へ送付）
  - Stage1 + base safe_benign条件のみ（Scenario 5: p1<0.15, defer<0.40）
  - Stage1 + base + 証明書ゲートのみ（Scenario 5+6）
  - Stage1 + base + 証明書ゲート + TLD制御（Scenario 5+6+7: 提案法）
  - 注: 実装にはScenario 8（高ML強制Stage3送付）も存在し、提案法に含まれる

#### 4.4 Stage3: AI Agent + ルールエンジンの性能
- Stage3単体性能（handoff集合 11,952件、全体の9.4%）
  - TP=1,686, FP=532, TN=8,975, FN=759
  - Precision: 76.01%, Recall: 68.96%, F1: 72.31%
  - 処理時間: 平均9.25秒、p50=8.31秒、p90=15.36秒、p99=28.65秒
  - ルール発火: 2,016件（16.9%）で少なくとも1ルールが発火
...
#### 4.5 システム全体の性能
- システム全体: TP=62,454, FP=535, TN=63,076, FN=1,157
  - Precision: 99.15%, Recall: 98.18%, F1: 98.66%
  - FPR: 0.84%, FNR: 1.82%
- Stage1+2 と Stage1+2+3 の比較（Stage3の純貢献）
  - Stage3なしの場合、Stage3 handoff分 (11,952件) は stage1_pred で代用 → FN増・FP減のトレードオフ
- FN削減とFP増加のトレードオフ（FN:FP比）
- 運用指標:
  - Stage3投入率: 9.4%（11,952/127,222）。自動判定率: 90.6%
  - Stage3遅延: 平均9.25秒、p50=8.31秒、p90=15.36秒、p99=28.65秒
...
- ルール閾値の調整
  - ドメイン知識ルールの判定閾値は、評価データ上の性能を参考に調整している
  - ルールの概念自体はデータ非依存だが、閾値の汎化性は未知のデータで検証が必要
...
## 検証済み数値リファレンス（2026-02-07 実データ突合）
...
| 項目 | 値 |
|------|-----|
| データ準備段階 Test | 127,754件 (Phishing 63,877 / Benign 63,877) |
| **実評価 Test** | **127,222件 (Phishing 63,611 / Benign 63,611)** |
| 脱落件数 | 532件（特徴量抽出時） |
...
- `artifacts/2026-02-02_224105/results/stage2_validation/eval_20260205_230157/eval_df__nALL__ts_20260205_230158.csv` (11,952行)

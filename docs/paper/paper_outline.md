...
URL・ドメイン特徴量によるML検出は広く研究されている。Sabir et al. (Nature Scientific Reports, 2022) はURL・HTML・証明書のハイブリッド特徴量をXGBoostで分類しF1=96.38%を報告した。証明書特徴量に特化した研究として、Torroledo et al. (Computers & Industrial Engineering, 2024) はPhishTank+TrancoデータをCensysで証明書取得し、証明書のみF1=0.77、ドメイン名追加でF1=0.89を達成している。Mohan et al. (Cluster Computing, 2023) はTLS 1.2/1.3トラフィックから復号不要で特徴抽出するアプローチを提案した。

- 共通課題: ブラックボックス性、誤り要因の体系的分析不足、分布変化への弱さ

### 2.2 LLMによるフィッシング検出

大型LLMの高精度性とコスト問題が議論されている。Alani et al. (MDPI Information, 2025) は21種のオープンソースLLMを4種のプロンプト技法で比較し、Few-shotで平均F1=82.6%（最高91.24%, Llama3.3-70B）を報告した。Wang et al. (arXiv, 2025) はスクリーンショット・ロゴ・HTML・URLの入力モダリティ別にLLM性能を評価し、商用LLMで93-95%を達成した。小型LLMでは、Koide et al. (arXiv, 2025) がQwen-2.5-1.5Bのファインチューニングで精度0.388→0.860への改善を報告している。

- 共通課題: 全件LLM処理の非効率性、再現性、説明性

### 2.3 マルチエージェント型フィッシング検出

LLMエージェントの協調による精度向上が近年活発に研究されている。Lin et al. (PhishDebate, arXiv, 2025) は4つの専門エージェント（URL構造・HTML構造・コンテンツ意味・ブランド偽装）+ Moderator + Judgeの構成でF1=94.14%を達成し、Single-Agent（F1=74.69%）からの大幅改善を示した。Kaddoura et al. (MultiPhishGuard, arXiv, 2025) はメタデータ・本文・URLを並列分析するマルチエージェント構成を提案した。

- 共通課題: GPT-4レベルの大型LLM前提、全件にLLM適用（トリアージなし）、ページ内容取得が必要

### 2.4 ハイブリッド手法と投入制御

MLによる絞り込み + LLM精査（トリアージ）のアプローチが提案されている。Champa et al. (Computers & Security, 2023) はLayer 1で軽量分類、Layer 2で詳細分析する二層構造を提案した。CSS2025（朝村ら, 2025）では二段設計（XGBoost → AI Agent）を実装したが、投入制御機構がなく全handoffをLLMが処理する非効率性があった。本研究は誤り確率推定層（Stage2）を導入した三段拡張により、この課題に対処する。

### 2.5 知識拡張と説明可能性

知識ベースの拡張による精度向上として、Li et al. (KnowPhish, USENIX Security, 2024) は20,000ブランドの多モーダル知識ベースを自動構築し、既存検出器にplug-and-playで統合する手法を報告した。説明可能性では、Javed et al. (EXPLICATE, arXiv, 2025) がML検出 + SHAP/LIME説明 + LLMによる自然言語変換の3層アーキテクチャを提案し、検出精度98.4%と説明品質94.2%を両立させた。

...
Recall は比率によらず一定だが、Precision は正規側の母数増大に伴い劣化する（base-rate fallacy）。逆算すると、Precision≥90% を維持するには実効比率を13:1以下に保つ必要がある（$p \geq 0.071$）。

CTログ規模（正規:フィッシング≈100:1〜1000:1）では、FPR=0.84% でも大量の偽陽性が生じるため、本カスケードの前段に候補フィルタ（ブランド類似度、TLD絞り込み等）を配置し、実効比率を引き下げることが不可欠である。この事前フィルタと、§5.5で述べるサイト稼働後のコンテンツ検証を組み合わせた運用設計が、実環境への展開における次の課題となる。

...
| Stage2 | なし | **LR（誤り確率予測）+ 証明書ゲートによるフィルタリング（新規）** |
...
**本論文の貢献**:
1. **Stage2（誤り予測層）の新設**: Stage1の判定信頼性をLR（ロジスティック回帰）で推定し、証明書属性ゲートと組み合わせてLLM処理対象を全体の9.4%に圧縮
2. **ドメイン知識ルールの統合**: Stage3において、LLM判定を証明書分野の専門知識で補完・修正する仕組みを実現
...
- 3段カスケードアーキテクチャ（図: CSS2025の図1を拡張）
  ```
  Stage1: XGBoost → 高速スクリーニング（52%を自動判定）
  Stage2: LR（誤り確率推定）+ 証明書ゲート → 信頼性フィルタ（38%を自動判定）
  Stage3: LLM + ドメイン知識ルール → 境界域精査（残り9.4%）
  ```
- CSS2025からの変更点を明示（2段→3段、Stage2のLR+ゲート層追加、Stage3のルール統合）
...
**3.3 Stage2: 誤り確率推定によるフィルタリング**（★新規セクション）
- **設計動機**: CSS2025の課題「全handoff候補にLLM処理」の解消
- **主メカニズム: LR（ロジスティック回帰）によるStage1誤り確率の推定**
  - Stage1のXGBoostと同じ特徴量 + Stage1出力由来の不確実性指標（entropy, uncertainty）を入力
  - 目的変数: Stage1の判定が誤りか否か（p_error）
  - 5-fold Stratified CV + Out-of-Fold予測で閾値選定
  - p_error が低い（Stage1が信頼できる）サンプルはLLM処理をスキップ
- **補助メカニズム: 証明書属性ゲート**
  - LR判定に加え、証明書の属性（CRL配布点、OV/EV認証、ワイルドカード、有効期間）に基づく決定的ゲート
  - これらは証明書業界の標準的な信頼性指標（認証局階層等）に基づく
- **TLDリスクフィルタ**: ゲートの適用をTLDの信頼度に応じて制御（危険TLDではゲートを抑制）
- フィルタ効果: handoff候補の74%をLLM処理前に判定（計算コスト大幅削減）
...
**4.3 Stage2: LR + ゲートの効果**（★新規）
- LRモデルの性能: p_error推定精度、閾値選定結果
- 圧縮効果: handoff候補の74%をフィルタ → Stage3への入力を大幅削減
- 自動判定の誤り率（auto error rate）: LR + ゲート併用時の精度
- LR単体 vs LR + ゲート併用の比較（ゲートの追加効果）
...
**5.1 各Stageの役割と相補性**
- Stage1: XGBoostによる高速スクリーニング（52%を自動判定）
- Stage2: LRが「Stage1を信頼できるか」を判断 + 証明書ゲートで補強（38%を自動判定）
- Stage3: LLM + ドメイン知識ルールで境界域を精査（9.4%を処理）
- **Stage2のLRは「メタ学習」**: Stage1の出力を入力として、Stage1自身の信頼性を推定する設計
...
- **データセット依存性**:
  - Stage2のLRおよびStage3のドメイン知識ルールは、当該データセットで訓練・調整されている
  - Stage2 LRは5-fold Stratified CVとOOF予測により過学習を抑制（標準的なML手法）
  - Stage2の証明書ゲートは業界標準知識（認証局階層、TLDリスク）に基づくが、閾値パラメータはデータ参照で調整
  - Stage3のドメイン知識ルールも同様に、概念は外部知識に根拠があるが閾値はデータ依存
  - 緩和策: (1) LRはCV手続きで汎化性能を検証 (2) ゲート・ルールの概念は外部知識ソース（Spamhaus、CERT報告等）に根拠がある (3) 今後の課題として別データセットでの検証を予定
...
- LR（誤り確率推定）+ 証明書ゲートによるStage2の新設でLLM処理を74%削減しつつ精度維持
- ドメイン知識ルールの統合により:
...
| 表3 | Stage2 LR + ゲートの効果（圧縮率・auto error rate）★新規 | 4.3 |
| 表4 | Stage3混同行列 | 4.4.1 |
...
### 新規執筆が必要
- **3.3 Stage2: LR + 証明書ゲート**（CSS2025に存在しない）
...
> CSS2025で提案した2段階ハイブリッドシステムに対し、Stage1の誤り確率を推定するLR層（Stage2）と、LLM判定を専門知識で補正するルール層（Stage3内）を新設した。Stage2のLRは「Stage1自身の信頼性」をメタ的に推定し、証明書属性ゲートと組み合わせることで、LLM処理量を74%削減しつつhandoff判定の精度を維持する。Stage3では、MLスコアが極めて低い（< 0.3）にもかかわらずフィッシングである事例群を、証明書品質・TLDリスク・ブランド偽装の複合的な専門知識により検知できることを示した。3段カスケード全体でF1 97.98%→98.66%を達成し、数値特徴量に依存するML単体では原理的に検知困難な領域を、メタ学習（LR）とドメイン知識（ルール）が補完しうることを示唆する。
